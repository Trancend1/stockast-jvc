// weather.jsx — Atmospheric scene illustrations (SVG)
//
// Six panoramic landscape scenes for the dashboard / Belanja Card hero:
//   · Cerah        — clear sunny morning
//   · Berawan      — soft cloud cover
//   · Hujan        — afternoon rain with rooftops
//   · Petir        — storm with lightning bolt
//   · Subuh        — pre-dawn / night with crescent moon + stars
//   · Berkabut     — misty foggy morning
//
// All scenes share the same ink anchor style as OnbDecor (1.5px stroke,
// warm warung silhouettes, brick + mint accents). Used as a full-bleed
// banner above stock items.
//
// Each component accepts:
//   width / height — SVG dimensions (defaults fit 16:5 banner aspect)
//   subtle         — if true, drops saturation for use inside cards
...
const W_GOLD   = "#E8C074";
const W_BLUE   = "#4A7EA0";
const W_DEEPSEA= "#0E1D56";

// Common wrapper
function Scene({ width = 320, height = 100, viewBox = "0 0 320 100", children, style }) {
  return (
...
// Shared rooftop village band — anchors all daytime scenes
function VillageBand({ inkOpacity = 0.65 }) {
  return (
    <g fill="none" stroke={W_INK} strokeWidth="1.4"
       strokeLinecap="round" strokeLinejoin="round" opacity={inkOpacity}>
...
// ─────────────── 1 · Cerah (sunny) ───────────────
function SceneCerah({ width, height }) {
  return (
...
// ─────────────── 2 · Berawan (cloudy) ───────────────
function SceneBerawan({ width, height }) {
  return (
    <Scene width={width} height={height}
...
// ─────────────── 3 · Hujan (rainy) ───────────────
function SceneHujan({ width, height }) {
  return (
    <Scene width={width} height={height}
...
// ─────────────── 4 · Petir (stormy) ───────────────
function ScenePetir({ width, height }) {
  return (
    <Scene width={width} height={height}
      ...
      {/* lightning bolt */}
      <path d="M 170 38 L 158 64 L 168 64 L 158 86 L 184 56 L 174 56 L 184 38 Z"
            fill={W_GOLD} stroke={W_INK} strokeWidth="1.4" strokeLinejoin="round" />
      {/* heavy rain */}
...
// ─────────────── 5 · Subuh (deep-sea night) ───────────────
function SceneSubuh({ width, height }) {
  return (
    <Scene width={width} height={height}
           style={{ background: "linear-gradient(180deg, #21295c 0%, #0e1d56 35%, #001f54 70%, #001235 100%)" }}>
      {/* stars */}
      <g fill="#c8e2ec">
      ...
      {/* village rooftops (silver light) */}
      <g fill="none" stroke="#7eb4cc" strokeWidth="1.3"
      ...
      {/* one warm window (the only warung still awake) */}
...
// ─────────────── 6 · Berkabut (foggy) ───────────────
function SceneBerkabut({ width, height }) {
  return (
    <Scene width={width} height={height}
      ...
      {/* faded village */}
      <g fill="none" stroke={W_INK} strokeWidth="1.2"
...
function WeatherScene({ kind = "cerah", width = 320, height = 100, style }) {
...
window.SkWeather = {
  Scene,
  SceneCerah, SceneBerawan, SceneHujan, ScenePetir, SceneSubuh, SceneBerkabut,
  WeatherScene, WEATHER_SCENES,
};
