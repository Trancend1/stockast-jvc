// icons.jsx — Custom Stockast icon set
// Single style: 1.5px stroke, slightly rounded caps. Paired with labels everywhere.
// All icons are 24×24 viewBox, color = currentColor, sized via `size` prop.

const ICON = ({ children, size = 18, stroke = 1.5 }) => (
...
const SkIcons = {
  ...
  check: (p) => (
    <ICON {...p}><path d="M4 12.5 9.5 18 20 6.5"/></ICON>
  ),
  close: (p) => (
  ...
  undo: (p) => (
    <ICON {...p}><path d="M9 14H5v-4M5 14a8 8 0 1 1 2.5 5.8"/></ICON>
  ),
  refresh: (p) => (
    <ICON {...p}>
  ...
  // Weather (animated morph capable)
  sun: (p) => (
    <ICON {...p}>
  ...
  spark: (p) => (   /* a quiet "AI" mark — not a sparkle, more like a quill nib */
    <ICON {...p}>
  ...
  moon: (p) => (
    <ICON {...p}><path d="M20 15A8 8 0 0 1 9 4a8 8 0 1 0 11 11Z"/></ICON>
  ),
  info: (p) => (
    <ICON {...p}><circle cx="12" cy="12" r="9"/><path d="M12 11v6M12 7.5v.5"/></ICON>
  ),
  ...
  sort: (p) => (
    <ICON {...p}><path d="M7 5v14M7 19l-3-3M7 19l3-3M17 19V5M17 5l-3 3M17 5l3 3"/></ICON>
  ),
  lock: (p) => (
    <ICON {...p}>
  ...
  clock: (p) => (
    <ICON {...p}><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3.5 2.5"/></ICON>
  ),
  bell: (p) => (
    <ICON {...p}>
  ...
  download: (p) => (
    <ICON {...p}><path d="M12 4v12M7 11l5 5 5-5M4 20h16"/></ICON>
  ),
  share: (p) => (
    <ICON {...p}>
  ...
  trendFlat: (p) => (
    <ICON {...p}><path d="M3 12h14M14 8l4 4-4 4"/></ICON>
  ),
  chartLine: (p) => (
    <ICON {...p}>
      <path d="M3 20V5"/>
  ...
  percent: (p) => (
    <ICON {...p}><circle cx="7" cy="7" r="2.5"/><circle cx="17" cy="17" r="2.5"/><path d="M5 19l14-14"/></ICON>
  ),
  receipt: (p) => (
    <ICON {...p}>
  ...
  // ─────── Status ───────
  checkCircle: (p) => (
    <ICON {...p}><circle cx="12" cy="12" r="9"/><path d="M8 12.5l3 3 5-6.5"/></ICON>
  ),
  closeCircle: (p) => (
    <ICON {...p}><circle cx="12" cy="12" r="9"/><path d="M8 8l8 8M16 8l-8 8"/></ICON>
  ),
  warnTriangle: (p) => (
    <ICON {...p}>
  ...
  // ─────── Weather (extras to existing sun/cloud/rain) ───────
  storm: (p) => (
    <ICON {...p}>
      <path d="M7 14h10a4 4 0 0 0 .8-7.9 6 6 0 0 0-11.7 1A4 4 0 0 0 7 14Z"/>
  ...
  star_small: (p) => (
    <ICON {...p}><path d="M12 5l1.5 4 4 .5-3 3 1 4-3.5-2-3.5 2 1-4-3-3 4-.5L12 5Z" fill="currentColor" stroke="none"/></ICON>
  ),
};

window.SkIcons = SkIcons;
