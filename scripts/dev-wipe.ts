...
// Service role key bypasses RLS — all rows are visible and deletable.
const db = createClient(url, key, {
  auth: { autoRefreshToken: false, persistSession: false },
});

// Ordered by FK dependency: children first, parents last.
// memberships has composite PK (user_id, organization_id) — no id column.
...
async function run() {
  console.log('Wiping all dev data...\n');
  let allOk = true;
  ...
  console.log('\nDB clean.');
  console.log('Next: clear browser localStorage → F12 → Application → Local Storage → localhost:3000 → Delete All\n');
